...
class RecordService(RecordServiceBase):
    ...
    def search_versions(
        ...
        **kwargs,
        ...
        # Pre-check access with plain ``read`` before running the versions search.
        # Downstream services such as invenio-rdm-records may run the search with
        # ``permission_action="read_deleted"`` so deleted records can resolve here
        # and still return an empty version list. Using ``permission_action`` for
        # this pre-check would fail early unless the caller also had
        # ``can_read_deleted``.
        #
        # This is similar to handling ``search`` permissions - ``can_search`` as
        # a pre-check and ``can_read`` as the actual permission check for each record
    ...
    def import_files(self, identity, id_, uow=None, **kwargs):
        ...
        # Read draft
        draft = self.draft_cls.pid.resolve(id_, registered_only=False)
        self.require_permission(identity, "update_draft", record=draft, **kwargs)
